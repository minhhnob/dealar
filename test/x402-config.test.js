...
test('getX402Environment defaults to Arc Testnet facilitator settings', () => {
  ...
  assert.equal(env.facilitatorUrl, 'https://conduitpay.xyz/api/x402');
...
test('buildX402RouteConfig returns x402 exact payment requirements for paid endpoints', () => {
  ...
  assert.equal(config['GET /v1/deals/search'].accepts.network, 'eip155:5042002');
