export const DEFAULT_DEALER_SOURCES = ['amazon', 'ebay', 'slickdeals'];
...
function pickDataset(query) {
  ...
  if (key.includes('dior') || key.includes('sauvage')) return CATALOG['dior sauvage'];
...
function enrichDeal(deal) {
  ...
  const retailerTrustBonus = { amazon: 4, slickdeals: 2, ebay: 0 }[deal.source] ?? 0;
  const directRetailerBonus = deal.source === 'amazon' ? 6 : 0;
...
export function searchDealerDeals({ query = 'Dior Sauvage', sources = DEFAULT_DEALER_SOURCES, region = 'us' } = {}) {
  const requested = Array.isArray(sources) ? sources : String(sources).split(',').map((s) => s.trim()).filter(Boolean);
  const wanted = requested.filter((source) => DEFAULT_DEALER_SOURCES.includes(source));
  const activeSources = wanted.length ? wanted : DEFAULT_DEALER_SOURCES;
  const results = pickDataset(query).filter((deal) => activeSources.includes(deal.source)).map(enrichDeal).sort((a, b) => b.score - a.score);
  const bestDeal = results[0];
  const safer = results.find((deal) => deal.risk === 'low') || bestDeal;
  const recommendation = {
    action: `consider_${bestDeal?.source || 'none'}`,
  ...
  const response = { query, region, sources: activeSources, bestDeal, results, recommendation };
...
export function quoteProduct({ query = 'Dior Sauvage' } = {}) {
  ...
  const safestDeal = priced.find((deal) => deal.source === 'amazon' && deal.risk !== 'high') || priced.find((deal) => deal.risk === 'low') || bestPrice;
...
export function searchDealerCoupons({ query = '', source } = {}) {
  const selected = source && DEFAULT_DEALER_SOURCES.includes(source) ? [source] : DEFAULT_DEALER_SOURCES;
